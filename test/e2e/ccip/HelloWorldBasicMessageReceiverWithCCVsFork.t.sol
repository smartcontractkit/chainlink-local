...
contract HelloWorldBasicMessageReceiverWithCCVsForkTest is Test {
    ...
    function test_helloWorldBasicMessageReceiverWithCCVs_fork() external {
        ...
        // Multiple CCV config: required default lane CCV marker + one optional existing on-chain contract.
        address[] memory requiredCCVs = new address[](1);
        requiredCCVs[0] = address(0);

        address[] memory optionalCCVs = new address[](1);
        optionalCCVs[0] = s_destinationNetwork.rmnProxyAddress;
        ...
        // Let lane defaults provide the CCV path; receiver config above enforces multi-CCV scenario shape.
