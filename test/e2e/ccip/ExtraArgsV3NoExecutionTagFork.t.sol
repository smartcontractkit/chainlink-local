...
contract ExtraArgsV3NoExecutionTagForkTest is Test {
    ...
    function test_noExecutionTag_forkManualExecutionPath() external {
        vm.selectFork(s_destinationFork);
        ...
        bytes[] memory ccvArgs = new bytes[](0);
        bytes memory extraArgs =
            s_encoder.encodeV3(200_000, 0, ccvs, ccvArgs, s_encoder.getNoExecutionAddress(), "", "", "");
        ...
        bytes32[] memory pendingMessageIds = s_forkSimulator.getPendingV2MessageIds();
        assertEq(pendingMessageIds.length, 1);
        assertEq(pendingMessageIds[0], messageId);

        bool attempted = s_forkSimulator.executePendingV2Message(messageId);
